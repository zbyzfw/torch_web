<template>
  <div style="margin:20px 50px 50px 50px">
    ...
    >
      ...
      <!--      <vxe-column field="column_name" title="微信名称" type="html">-->
      ...
      <vxe-column field="rate" title="操作" fixed="right" width="260px">
        <template #default="{ row }">
          ...
          <vxe-button type="text" icon="el-icon-star-off" status="warning" title="普查" @click="pointDetail(row)" />
          <vxe-button type="text" icon="fa fa-calculator" status="success" title="普查结果" @click="scoreDetail(row)" />
    ...
    <!--    : '添加微信  - '+ $store.state.city.siteInfos.label + ' - ' + $store.state.city.classifyInfo.label"-->
    ...
    <vxe-modal
      v-model="showEdit"
      :title="selectRow
        ? '编辑站点 - ' + $store.state.city.siteInfos.label
        : '添加微信  - '+ $store.state.city.siteInfos.label"
      width="600"
      min-width="400"
      min-height="300"
    ...
    >
      <template #default>
        ...
        <vxe-form :data="formData" :rules="formRules" title-align="right" title-width="100" @submit="submitEvent">
          ...
          <vxe-form-item field="" title="微信名称: " :item-render="{}" :span="20">
            <template #default="{ data }">
              <vxe-input v-model.trim="data.column_name" placeholder="请输入微信名称" />
            </template>
          </vxe-form-item>

          <vxe-form-item field="" title="微信biz: " :item-render="{}" :span="20">
            <template #default="{ data }">
              <vxe-input v-model="data.column_url" placeholder="请输入微信biz" />
            </template>
          </vxe-form-item>
...
import {
  ...
  queryLingCaiXinMeiTiByCityIdAndSiteClassify,
...
import { queryNewMediaByCityIdSiteClassity, queryNewMediaByCityIdSiteClassityNameBizId } from '@/api/pucha'
...
export default {
  ...
  methods: {
    ...
    search(page) {
      ...
      if (filterName) {
        queryLingCaiWeixinByCityIdAndSiteClassify(this.$store.state.city.city_id, this.$store.state.city.site_classify, filterName,
          ...
          // state.siteList = state.siteList.slice((state.tablePage.currentPage - 1) * state.tablePage.pageSize, state.tablePage.currentPage * state.tablePage.pageSize)
        }).catch(err => {
          // console.log("queryLingCaiXinMeiTiByCityIdAndSiteClassify获取数据失败err=======> " + err)
          this.$store.state.city.loading = false
          this.$XModal.message({ content: '获取微信数据失败', status: 'error' })
        })
    ...
    getTableData(page) {
      ...
      queryNewMediaByCityIdSiteClassity(this.$store.state.city.city_id, this.$store.state.city.site_classify, this.$store.state.city.tablePage.currentPage, this.$store.state.city.tablePage.pageSize).then(response => {
    ...
    submitEvent() {
      ...
      setTimeout(() => {
        ...
        if (this.selectRow) {
          ...
          // this.$XModal.alert({content: '暂时不能添加微信'})
          ...
          insertNewMediaColumn(this.formData).then(response => {
            ...
            // console.log("站点数据添加失败原因==================》: " + response)
            // console.log("站点 insertData(params) 添加数据失败")
            this.$XModal.message({ content: '数据已存在，添加失败', status: 'error' })
            this.getTableData()
          }).finally(msg => {
            this.$XModal.message({ content: '数据已存在，添加失败', status: 'error' }
            )
    ...
    // 查看网站测评合格不合格详情页面
    scoreDetail(row) {
      // console.log("微信跳转至评分概览===========> " + JSON.stringify(row))
      // this.$router.push("/site/scoreDetail")
      this.$router.push({
    ...
    getFilterNewMedia(filterName1, web_bsm) {
      ...
      queryNewMediaByCityIdSiteClassityNameBizId(this.$store.state.city.city_id, this.$store.state.city.site_classify, this.filterName1,
