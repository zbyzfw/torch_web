...
import { queryCityList, queryCityTree } from '@/api/t_city'
...
export default {
  ...
  methods: {
    ...
    cityList() {
      queryCityTree().then(response => {
        this.cityTree = response
        // console.log("城市树的数据为： " + JSON.stringify(response))
      }).catch(err => {
        // console.log("获取城市树失败原因： " + err)
        this.$XModal.message({ content: '获取城市树数据失败', status: 'error' })
      })
