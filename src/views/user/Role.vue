  ...
  <div style="margin:20px 50px 50px 50px">
    ...
    <vxe-toolbar>
      <template #buttons>
        ...
        <vxe-button icon="fa fa-plus" status="primary" @click="insertEvent()">添加角色</vxe-button>
      </template>
    </vxe-toolbar>
    ...
    >
      <template #default>
        ...
        <vxe-form ref="formData" :data="formData" :rules="formRules" title-align="right" title-width="100">
          <vxe-form-item field="name" title="角色名称: " :item-render="{}" :span="20">
            <template #default="{ data }">
              <vxe-input v-model.trim="data.name" placeholder="请输入角色名称" />
            </template>
          </vxe-form-item>
...
export default {
  ...
  methods: {
    ...
    // 打开页面时加载数据
    getRoleData(currentPage, pageSize) {
      getRole(currentPage, pageSize).then(response => {
        // console.log(("Role加载全部数据： " + JSON.stringify(response)))
        // const data = response
